...
Swarm.prototype._onAnnounceStarted = function (params, peer, id) {
  ...
  if (params.left === 0) this.complete += 1
    ...
    this.incomplete += 1
    const endpoint = new URL(this.license.verifer.endsWith('/')
      ? `${this.license.verifer}balances/${params.requestId}:spend`
      : `${this.license.verifer}/balances/${params.requestId}:spend`)
    fetch(endpoint.href, { method: 'POST', body: this.license.amount.toString() })
      .then(_checkStatus)
      .then(
        this.peers.set(id, {
          type: params.type,

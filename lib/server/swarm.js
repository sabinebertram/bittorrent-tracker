...
function Swarm (infoHash, license, server) {
  var self = this
...
Swarm.prototype.announce = function (params, cb) {
  ...
  if (params.event === 'started') {
    try {
      self._onAnnounceStarted(params, peer, id, self._verify)
    } catch (error) {
      cb(error)
    }
...
Swarm.prototype._onAnnounceStarted = function (params, peer, id, cb) {
  if (peer) {
  ...
  if (params.downloaded === 0) {
    ...
    cb(verifyParams).catch(e => console.log(`Verify receipt: ${e.message}. RequestID: ${params.requestId}`))
  }

  if (params.downloaded !== 0 & params.left === 0) this.complete += 1
...
Swarm.prototype._onAnnounceCompleted = function (params, peer, id) {
  if (!peer) {
    ...
    return this._onAnnounceStarted(params, peer, id, this._verify) // treat as a start
  }
  if (peer.complete) {
...
Swarm.prototype._verify = async function (params) {
